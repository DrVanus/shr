...
//  CSAI1
//
//  Final version for using a standard project API key with the Assistants API,
//  including the required Authorization, OpenAI-Beta, and Content-Type headers.
//  The OpenAI-Organization header has been removed for a single-org setup.
...
struct AITabView: View {
    ...
    // Your project API key (must be a standard key starting with "sk-")
    private let openAIKey = """
    sk-your_project_api_key_here
    """
    
    // For single-org setups, we are not using an explicit OpenAI-Organization header.
...
extension AITabView {
    ...
    /// The core Assistants API call with updated JSON decoding.
    private func fetchAIResponse(for userInput: String) async throws -> String {
        ...
        for _ in 1...20 {
            ...
            if statusRes.status.lowercased() == "succeeded" || statusRes.status.lowercased() == "completed" {
                ...
                // Updated decoding to match the actual JSON response structure
        ...
        guard let reply = assistantReply else {
