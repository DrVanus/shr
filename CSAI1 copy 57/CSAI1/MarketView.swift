...
class MarketViewModel: ObservableObject {
    ...
    // When sortField is .none, we preserve the API's default order (market cap descending)
    ...
    // Tapping the same column header resets sort to default (API order)
    ...
    private func sortCoins(_ arr: [MarketCoin]) -> [MarketCoin] {
        // If no sort is selected, preserve the API's default order (market cap descending)
        guard sortField != .none else { return arr }
        return arr.sorted { (lhs: MarketCoin, rhs: MarketCoin) -> Bool in
            switch sortField {
            case .coin:
...
struct MarketView: View {
    ...
    private func headerButton(_ label: String, _ field: SortField) -> some View {
        Button {
            ...
            .padding(6)
            .background(RoundedRectangle(cornerRadius: 8).fill(Color.white.opacity(vm.sortField == field ? 0.3 : 0.1)))
        }
...
private func coinImageView(symbol: String, urlStr: String?) -> some View {
    ...
    if let geckoUrlStr = urlStr, let geckoUrl = URL(string: geckoUrlStr) {
        return AnyView(
            AsyncImage(url: geckoUrl) { phase in
                if let image = phase.image {
                    ...
                    if let ccURLStr = cryptoCompareLogos[upperSymbol], let ccURL = URL(string: ccURLStr) {
    ...
    } else if let ccURLStr = cryptoCompareLogos[upperSymbol], let ccURL = URL(string: ccURLStr) {
        return AnyView(
            AsyncImage(url: ccURL) { phase in
                if let image = phase.image {
                    image.resizable()
        ...
        return defaultImage(for: symbol)
    }
}

private func defaultImage(for symbol: String) -> AnyView {
