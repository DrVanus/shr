        ...
        HStack {
            ...
            case -1:
                Text("Error loading price")
                    ...
                    .font(.system(size: 20, weight: .bold))
            default:
                // Comma-based price: up to 8 decimals if <1, else 2 decimals
    ...
    private var tradeForm: some View {
        VStack(spacing: 12) {
            ...
            HStack(spacing: 8) {
                ...
                // Removed Spacer() so percentage buttons are closer to the + button
                HStack(spacing: 4) {
...
class PriceViewModel: ObservableObject {
    ...
    func updateSymbol(_ newSymbol: String) {
        ...
        // Reset to 0 so it shows "Loading..." while we fetch
        currentPrice = 0
        fetchPrice()
    }
