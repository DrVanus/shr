...
/// Keep this style in one place only. PortfolioChartView will reference it, so don’t duplicate it there.
...
/// Keep this enum in one place only. PortfolioChartView references it.
...
/// If you no longer need this older geometry-based line chart, remove it.
...
// MARK: - CoinCardView
/// A smaller dimension card used in Trending/Top Gainers/Losers
...
struct HomeView: View {
    ...
    var body: some View {
            ...
            NavigationView {
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        // Reference your new advanced chart:
                        PortfolioChartView()
        ...
        .onAppear {
            // e.g. marketVM.fetchCoins() if it exists
            fearGreedVM.fetchIndex()
            newsVM.fetchNews()
        }
    }
...
extension HomeView {
    ...
    // Coin Icon Helper (used in watchlist)
    private func coinIconView(for coin: MarketCoin, size: CGFloat) -> some View {
        Group {
            if let imageUrl = coin.imageUrl, let url = URL(string: imageUrl) {
                AsyncImage(url: url) { phase in
